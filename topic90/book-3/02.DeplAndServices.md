...
In a DevOps work environment, automation of application rollout and rollback is essential. Kubernetes Deployment resource support these requirements.
...
~~~
$ oc get po -o wide
❯ oc delete pod nginx-depl-9b59dbbfc-4grnf      (Use your pod name)
$ oc get po -o wide
~~~
...
Services serve as a stable interface to the set of pods managed by a replicaset/deployment, due to the dynamic nature of pod IP address allocation.
...
~~~
Create the file's resources. Notice that the service is created, but the deployment is unchanged:)
~~~
$ oc apply -f nginx-deployment.yaml
$ oc get service                   
~~~
There may be some default services, as well as yours.
Run the above apply command again, and this time it responds that the service and deployment are unchanged. 
...
Ln general, pods communicate via services. Because Kubernetes performs domain name resolution, a pod can reference a service by its name rather than IP address. To demo this, open a terminal session with one of the httpd pods:
~~~
...
A service __load-balances__ between its associated pods. To prove this, we will add a line to the bottom of the NGINX web page that shows the its IP address of its pod.__For each__ of the NGINX pods, complete the following sequence:
~~~
