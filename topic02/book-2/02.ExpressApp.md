...
The second component of our application is the Express Web app. It also needs a deployment and a service resource. In general, we can have more than one manifest in a single YAML file by separating them with triple-dash (---). Create the file `deploy-express.yaml`:
~~~
apiVersion: apps/v1
kind: Deployment
metadata:
...
~~~
The new resources created include: a deployment, a replicaset, two pods (arbitrary number), and a service:
~~~
$ oc get deploy                    
$ oc get rs                    
$ oc get svc
$ oc get po 
~~~
The two new pods are running containers with the Mongo Express Docker image, which we used in an earlier Docker lab. The image contains a Node-Express web app that listens on port 8081, and it expects three environment variables, namely:
+ ME_CONFIG_MONGODB_SERVER - set to the domain name of the database server, which, in a Kubernetes cluster, is the __name of the service__ whose endpoint (pod) is running the MongoDB container - mongo-svc. 
